namespace intell.portal {
    ...
    export class TaskbarGroup {
        constructor(element?: HTMLElement) {
            ...
            if (element != null) { let c = TaskbarGroup.getItem(element); if (c != null) return c; }
            ...
            TaskbarGroup.setItem(element, this);
        ...
        /** Gets the taskbar of the group. */
        get parent() { return this.getPrivate().parent }

        /** Gets an array containing all option items. */
        get shortcuts() { return this.getPrivate().shortcuts.slice() }
        ...
        set default(newValue) {
            ...
            ctrl.toggle(__private.elementHeader, !newValue);
        }
        //#endregion

        //#region methods
        /** Adds a new shortcut to the end of the list for this group. */
        add(item: TaskbarShortcut) {
            ...
            if (parent != null) {
                ...
                parentPrivate.shortcuts.push(item);
                parentPrivate._needsResortOptions = true;
            }
            __private.shortcuts.push(item);
        ...
        remove() {
            ...
            // 1. Removes children from parent.items (taskbar)
            // 2. Removes group from parent.group (remove this group from its parent)
            // 3. clears group.parent

            // --1--
            const shortcuts = parentPrivate.shortcuts;
            for (let i = 0; i < __private.shortcuts.length; i++) shortcuts.remove(__private.shortcuts[i]);
        ...
        //@ts-ignore
        declare getPrivate(def?: object): TaskbarGroupPrivate;

        //@ts-ignore
        declare static getItem(element: HTMLElement): TaskbarGroup; //@ts-ignore
        declare static setItem(element: HTMLElement, ctrl: TaskbarGroup): TaskbarGroup;
    }

    ctrl.template.inherit(TaskbarGroup);
}